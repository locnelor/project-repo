...
<template>
  <TabsRoot v-model="currentTab" class="bg-background-deep border-border overflow-hidden rounded-b-xl border-t"
      ...
      <div class="flex w-full items-center justify-between text-[13px]">
        <TabsList class="relative flex">
          <template v-if="open">
            ...
            <TabsTrigger v-for="(tab, index) in tabs" :key="index" :value="tab.label"
    ...
    <div :class="`${open ? 'h-[unset] max-h-[80vh]' : 'h-0'}`"
