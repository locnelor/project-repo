...
:root {
  ...
  /* 字体 */
  --q-font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
  --q-font-size-sm: 12px;
...
```vue
<template>
  <QButton :loading="loading" @click="handleSubmit">
    提交
  </QButton>
</template>
...
<template>
  ...
  <!-- ❌ 不推荐 - 内联复杂逻辑 -->
  <QButton @click="loading = true; submitForm().finally(() => loading = false)">
    提交
  </QButton>
</template>
```
