  ...
  const execute = async (...args: any[]): Promise<T> => {
    ...
    const executeRequest = async (): Promise<T> => {
      try {
        ...
        const result = await Promise.race([requestFn(...args), timeoutPromise]);
...
export function useUserApi() {
  ...
  // 创建用户
  const { loading: createLoading, execute: createUser } =
    usePost<User>("/api/users");

  // 更新用户
  const { loading: updateLoading, execute: updateUser } =
    usePut<User>("/api/users/:id");

  // 删除用户
  const { loading: deleteLoading, execute: deleteUser } =
  ...
  // 分配角色
  const { loading: assignRoleLoading, execute: assignRole } = usePost<void>(
    "/api/users/:id/roles",
  );
...
export function usePermission() {
  ...
  // 过滤有权限的菜单项
  const filterMenus = <
    T extends { permissions?: string[]; roles?: string[]; children?: T[] },
  >(
    menus: T[],
  ): T[] => {
    return menus
...
<template>
  <div>
    ...
    <!-- 用户表单 -->
    <QForm :model="formData" @submit="handleSubmit">
...
export function useUserManagement() {
  const { getUserList, createNewUser, updateUserById, deleteUserById } =
    useUserApi();
  ...
  const canCreate = computed(() =>
    checkAccess({ permissions: ["user:create"] }),
  );
  const canEdit = computed(() => checkAccess({ permissions: ["user:edit"] }));
  const canDelete = computed(() =>
    checkAccess({ permissions: ["user:delete"] }),
  );
...
export function useSearch() {
  ...
  const {
    ...
    loading,
    execute: search,
  } = useRequest((keyword: string) => searchApi(keyword));
