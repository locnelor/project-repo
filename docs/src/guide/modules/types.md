 ...
 * 函数返回类型
 */
export type FunctionReturn<T> = T extends (...args: any[]) => infer U
  ? U
  : never;
 ...
 * 重命名键类型
 */
export type Rename<T, K extends keyof T, N extends string> = Except<T, K> &
  Record<N, T[K]>;
 ...
 * 错误处理器类型
 */
export type ErrorHandler<T extends ApiError = ApiError> = (
  error: T,
) => void | Promise<void>;
...
export interface UpdateUserData
  extends Partial<Omit<UserProfile, "id" | "createdAt" | "updatedAt">> {
  roleIds?: ID[];
}
